...
class YamcsCommunication(App):
    ...
    def __init__(self, airliner_map, yamcs_address='localhost',
                 yamcs_port=8090, yamcs_instance='pyliner'):
        """
        Args:
        ...
        """
        super(YamcsCommunication, self).__init__()
        ...
        self.yamcs_address = yamcs_address
        ...
        # TODO:Replace with yamcs parameter streaming
        # Receive Telemetry
    ...
    def receive_callbacks(self):
        ...
        #     self.send_websocket_message('http://localhost:8090/api/websocket'))

    def issue_command(self, command_path: str):
        command = self.processor.issue_command(
            command_path, args={}
        )
        print("Issued", command)

    def __connect(self):
        client = YamcsClient(f'{self.yamcs_address}:{self.yamcs_port}')
        self.processor = client.get_processor(self.yamcs_instance, "realtime")
    ...
    def attach(self, vehicle):
        super(YamcsCommunication, self).attach(vehicle)
        self.control_thread = RealTimeThread(
            name='ControlRotateThread', target=self.control_rotate,
            every=YamcsCommunication.CONTROL_ROTATE_EVERY)
    ...
    def detach(self):
        ...
        super(YamcsCommunication, self).detach()
    ...
    def control_grant(self):
        ...
        for app_name in get_or_stop():
            ...
            try:
                ...
                )).first(YamcsCommunication.CONTROL_ACK_WAIT).result
                ...
                self.error('{} did not acknowledge control handoff within '
                           '{} seconds.'.format(
                                app_name, YamcsCommunication.CONTROL_ACK_WAIT))
                self.control_revoke()
                continue
            else:
    ...
    def get_app(self, app_ops_name, ops_names):
        apps = self.airliner_map[ops_names[0]]['apps']
    ...
    def _get_airliner_op(self, op_path):
        ...
        # r = self.get_app(ops_names[1], ops_names)
        # print(f"result1:{r}")
    ...
    def _on_recv_yamcs_telemetry(self, tlm: ParameterData):
        """ Callback for TO socket listener
        ...
        pv: [ParameterValue] = tlm.parameters

        # print(f"_on_recv_yamcs_telemetry-->{pv[0].eng_value}")
        # print(f"subs-->{self.subscribers}")

        for sub in self.subscribers:
            if sub['op_path'] == pv[0].name:
                print("Value Update")
                print(f'sub-->{sub}')
                callback = sub['callback']
                telemItem: _Telemetry = sub['telemItem']
                telemItem.update(value=pv[0].eng_value, time=pv[0].reception_time, obj=None)
                if callable(callback):
                    callback(self._telemetry[tlm])

        # # Iterate over subscribed telemetry to check if we care
        # for subscribed_tlm in self.subscribers:
        #     print('subscribed_tlm-->', subscribed_tlm)
        #     if int(subscribed_tlm['airliner_mid'], 0) == \
        #             int(tlm_pkt.PriHdr.StreamId.data):
        #         # Get pb msg for this msg
        #         op_path = subscribed_tlm['op_path']
    ...
    def subscribe(self, tlm_item, callback = 0):
        ...
        # FIXME: Each subscriber should really be an object, not a dict with magical strings
        ...
        self.processor.create_parameter_subscription(
            tlm_item, on_data=self._on_recv_yamcs_telemetry
        )
...
# def update_callback(func: function):
#     newTelemetry.update()
